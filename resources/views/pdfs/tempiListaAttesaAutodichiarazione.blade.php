...
<img alt="logo" class="icon" src="{{ asset(config('bootstrap-italia.logo.url')) }}">
    ...
    <h3>Indicatore 1.2 - Favorire la presa in carico dei pazienti affetti da patologie cronico-degenerative e oncologiche (D.L. 73/2024)</h3>
    <p>Numero di agende dedicate ai PTDA aziendali: {{ $numero_agende }}</p>
    <p>Num prest. di controllo prescritte dallo specialista ambulatoriale (Anno {{$anno}}): {{ $prestazioni_specialista_riferimento }}</p>
    <p>Num prest. di controllo prescritte dallo specialista ambulatoriale (Anno {{$anno-1}}): {{ $prestazioni_specialista_precedente }}</p>
    <p></p>
